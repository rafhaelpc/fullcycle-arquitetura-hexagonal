...
export default async function(productService : ProductServiceInterface, productId : string) {
  let answers : any = {};
  let response : any;
  ...
  const result = await ProductCLI.run(productService, 'get', productId, null, null);
  console.log(result);
